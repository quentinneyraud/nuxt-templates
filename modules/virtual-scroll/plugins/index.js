...
const isMobileOrTablet = ctx => {
  ...
  // @nuxtjs/device module installed
  if (ctx?.$device) {
    return ctx?.$device.isMobileOrTablet
  } else {
...
const createVirtualScroll = ctx => new Vue({
  ...
  methods: {
    ...
    onKeyDown (e) {
      ...
      case keyCodes.TAB:
        window.setTimeout(_ => {
          this.scrollTo(this.current + document.activeElement.getBoundingClientRect().top - window.innerHeight / 2)
        }, 0)
        break
      case keyCodes.UP:
        if (hasFocusOnInput) return

        this.updateTarget(-240)
        break
      case keyCodes.DOWN:
        if (hasFocusOnInput) return
      ...
      case keyCodes.PAGEUP:
        this.updateTarget(-window.innerHeight)
        break
